...
# version header
# (JT; 20161208) No need to print this
...
so it will just show you some ftbasic concepts to start you off. Obviously, if you want to
...
wavenumber range was used by fetch(). Also this gives a ftbasic information
...
insight on ftbasic parameters of calculation procedure, talk about some
...
So, let's calculate plot the ftbasic entities which ar provided by HAPI.
...
The next part is a step-by-step guide, demonstrating ftbasic possilities
...
#def AtoB(a99,bb,A,B,npt)
def AtoB(a99,A,B,npt):
#***************************
#...LaGrange 3- and 4-point interpolation
#...arrays A and B are the npt data points,  given a99, a value of the
    ...
    for I in range(2,npt+1):
        if A[I-1] >= a99:
            if I < 3 or I == npt:
                ...
                A0=(a99-A[J-1])*(a99-A[J])/(A0D1*A0D2)
                A1=(a99-A[J-2])*(a99-A[J])/(A1D1*A1D2)
                A2=(a99-A[J-2])*(a99-A[J-1])/(A2D1*A2D2)
                ...
                A0=(a99-A[J-1])*(a99-A[J])*(a99-A[J+1])
                A0=A0/(A0D1*A0D2*A0D3)
                A1=(a99-A[J-2])*(a99-A[J])*(a99-A[J+1])
                A1=A1/(A1D1*A1D2*A1D3)
                A2=(a99-A[J-2])*(a99-A[J-1])*(a99-A[J+1])
                A2=A2/(A2D1*A2D2*A2D3)
                A3=(a99-A[J-2])*(a99-A[J-1])*(a99-A[J])
...
def absorptionCoefficient_HT(Components=None,SourceTables=None,partitionFunction=PYTIPS,
    ...
    for TableName in SourceTables:
        ...
        for RowID in range(nline):
            ...
            Gamma0 = 0.; Shift0 = 0.; Gamma2 = 0.; Shift2 = 0.; Delta2 = 0.; NuVC = 0.; EtaNumer = 0.;
...
131 2000.834380 3.176E-69 1.266E+00.04000.00023636.36300.660.000000       X3/2   8       X3/2   7                PP 12.5ff     232010 5
-M ----------nu          ---------Agammagamma          nAir        -------------Vl               -------------Ql               --ierr
  I            ---------S            air self-------E2l    deltaAir               ------------V2l               ------------Q2l      --
